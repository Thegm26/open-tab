...
export class SupabaseOpenTabStore {
  ...
  private orderRow(row: any): Order { return { id: row.id, scenarioId: row.scenario_id, merchantId: row.merchant_id, totalCents: Number(row.total_cents), roundupContributionCents: Number(row.roundup_contribution_cents ?? 20), openTabCents: Number(row.open_tab_cents), customerTenderCents: Number(row.customer_tender_cents), remainingTenderCents: Number(row.remaining_tender_cents), status: row.status, refundableCents: Number(row.refundable_cents), refundedCents: Number(row.refunded_cents), customerRefundedCents: Number(row.customer_refunded_cents), poolRefundedCents: Number(row.pool_refunded_cents), createdAt: new Date(row.created_at) }; }
  ...
  async createOrder(input: any) { const out = await this.rpc<any>("ot_create_order", { p_scenario: input.scenarioId, p_merchant: input.merchantId, p_total: input.totalCents, p_roundup_cents: input.roundupContributionCents ?? 20, p_key: input.idempotencyKey, p_hash: hashToken(JSON.stringify(input.requestFingerprint ?? input)) }); return { order: this.orderRow(out) }.order; }
